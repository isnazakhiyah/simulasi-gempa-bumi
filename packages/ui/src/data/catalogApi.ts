...
function buildQuery(filters: CatalogEventFilters) {
  ...
  if (filters.minMag !== undefined) {
    params.set('minMag', String(filters.minMag));
  }

  if (filters.maxMag !== undefined) {
    params.set('maxMag', String(filters.maxMag));
  }
...
export async function fetchCatalogEvents(
  filters: CatalogEventFilters,
) {
  const response = await fetch(
    `${API_BASE_URL}/api/v1/catalog/events${buildQuery(filters)}`,
  );

  return parseApiResponse<CatalogListResponse>(
    response,
  );
}

export async function fetchCatalogEventDetail(
  id: string,
) {
  const response = await fetch(
    `${API_BASE_URL}/api/v1/catalog/events/${id}`,
  );

  return parseApiResponse<CatalogEventDetail>(
    response,
  );
}